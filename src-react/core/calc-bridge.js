...
// PS 재원 실효율 — **단순 10%**다(psPoolCircular: false, 2026-09-17 사용자 결정).
// 근거: 2026-09-16 가결된 임단협의 **애드백(Add-back)** — "성과급 지급 비용으로 영업이익이
// 줄더라도 차기 PS를 산정할 때 그 비용을 다시 영업이익에 더한다"(헤럴드경제 9/15 · 서울신문 9/16).
// 되더한 영업이익에 10%를 걸면 공시 영업이익 × 10%로 돌아온다 — 순환 차감(÷1.1)이 상쇄된다.
// 같은 방향 축 하나 더: 솔리다임 제외 종료(2025년 실적분이 마지막 — 실측 9.53%의 사유였다).
// **9.09%·11.11%를 적은 보도는 없다.** 노사·전 매체가 쓰는 유일한 숫자가 '영업이익의 10%'다(§2).
// 남은 긴장: 공시 영업이익이 이미 차감 후라는 DART 대조(2026-09-15)는 유효하고, 그 위에서
// 애드백을 *동년 자기참조*로 읽으면 OP/9 = 11.11%가 된다. 보도 문구가 '**차기**'라 택하지 않았다.
// 재판정 방아쇠: 2027년 초 재원 발표 — 실제 재원 ÷ 공시 영업이익을 10%와 대라.
// 플래그를 true로 되돌리면 ÷1.1(9.09%)로 돌아간다. 근거 전문은 _psPoolCircularWhy ·
// docs/sessions/2026-09-17-hynix-addback-vote.md.
...
// 2026 임단협 — **2026-09-16 가결**(찬성 57.08%). PS 지급 분해:
// 당해 80%(현금 50 + 주식 30) + 이연 20%(주식, 1년 10 + 2년 10). '현금 50 · 주식 50'과
// 모순이 아니라 주식 50의 내부 분해다. 적용은 2027년 초 지급분(= 2026년 실적분)부터.
// 1차안(현금 40 + 주식 60)은 2026-08-25 투표에서 25표 차로 부결됐다 — 9/15~16 재투표로 뒤집혔다.
